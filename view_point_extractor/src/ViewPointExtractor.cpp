    ...
    std::vector<view_point_extractor::ViewPoint> &view_point_vec)
{
  ...
  std::cout << "Start call pointcloud2_to_object_vec_converter to get objects..." << std::endl;
  ...
  if(!getObjects(objects))
  {
    std::cout << "ViewPointExtractor::getMultiView : " <<
      "get objects failed!" << std::endl;

    return false;
  }
  std::cout << "Finish call pointcloud2_to_object_vec_converter to get objects!" << std::endl;

  if(objects.size() == 0)
  {
    std::cout << "ViewPointExtractor::getMultiView : " <<
      "No object found! Please get view_ponit_vec later." << std::endl;

    return true;
  }
  std::cout << "Got " << objects.size() << " objects!" << std::endl;

  std::vector<sensor_msgs::PointCloud> trans_objects;
  if(!transPointCloud2VecToPointCloudVec(objects, trans_objects))
  {
    std::cout << "ViewPointExtractor::getMultiView : " <<
      "trans pointcloud2 to pointcloud failed!" << std::endl;
  ...
  std::cout << "Start get sampling point cloud vec..." << std::endl;
  ...
  std::cout << "Finish get sampling point cloud vec!" << std::endl;

  std::cout << "Start call grnet_detect to get full point cloud2 vec..." << std::endl;
  ...
  std::cout << "Finish call grnet_detect to get full_point_cloud2!" << std::endl;
...
bool ViewPointExtractor::getObjects(std::vector<sensor_msgs::PointCloud2> &objects)
{
  ...
  std::cout << "Get Objects from pointcloud2_to_object_vec_converter!" << std::endl;
  ...
  for(size_t i = 0; i < point_cloud2_vec.size(); ++i)
  {
    if(!sensor_msgs::convertPointCloud2ToPointCloud(point_cloud2_vec[i], point_cloud_vec[i]))
    {
      std::cout << "ViewPointExtractor::transPointCloud2VecToPointCloudVec : " <<
        "trans point_cloud2[" << i << "] failed!" << std::endl;
  ...
  for(size_t i = 0; i < point_cloud_vec.size(); ++i)
  {
    if(!sensor_msgs::convertPointCloudToPointCloud2(point_cloud_vec[i], point_cloud2_vec[i]))
    {
      std::cout << "ViewPointExtractor::transPointCloudVecToPointCloud2Vec : " <<
        "trans point_cloud[" << i << "] failed!" << std::endl;
    ...
    const int &sampling_point_num)
{
  ...
  for(size_t i = 1; i < sampling_point_num; ++i)
  {
    ...
    // std::cout << "running at " << i+1 << "/" << sampling_point_num << " episosde" << std::endl;
    ...
    const size_t &sampling_point_num)
{
  if(source_point_cloud_vec.size() == 0)
  {
    std::cout << "ViewPointExtractor::getSamplingPointCloudVec : " <<
      "source_point_cloud_vec is empty!" << std::endl;
    ...
    sensor_msgs::PointCloud2 &full_point_cloud2)
{
  ...
  if(!grnet_detector_client_.call(get_full_pointcloud_serve))
  {
    std::cout << "ViewPointExtractor::getFullPointCloud2FromPartialPointCloud2 : " <<
      "call grnet_detector failed!" << std::endl;
  ...
  for(size_t i = 0; i < partial_point_cloud2_vec.size(); ++i)
  {
    if(!getFullPointCloud2FromPartialPointCloud2(partial_point_cloud2_vec[i], full_point_cloud2_vec[i]))
    {
      std::cout << "ViewPointExtractor::getFullPointCloud2VecFromPartialPointCloud2Vec : " <<
        "get full pointcloud2 from partial pointcloud2 failed!" << std::endl;
    ...
    view_point_extractor::ViewPoint &next_best_view_point)
{
  ...
  {
    std::cout << "ViewPointExtractor::getMaxDistSumFullPoint : input partial_point_cloud is empty!" << std::endl;
    return false;
  }
  if(full_point_cloud.points.size() == 0)
  {
    std::cout << "ViewPointExtractor::getMaxDistSumFullPoint : input full_point_cloud is empty!" << std::endl;
    return false;
  }
    ...
    view_point_extractor::ViewPoint &next_best_view_point)
{
  ...
  if(partial_point_cloud.points.size() == 0)
  {
    std::cout << "ViewPointExtractor::getNextBestViewPoint : input partial_point_cloud is empty!" << std::endl;
    return false;
  }
  if(full_point_cloud.points.size() == 0)
  {
    std::cout << "ViewPointExtractor::getNextBestViewPoint : input full_point_cloud is empty!" << std::endl;
    return false;
  }
